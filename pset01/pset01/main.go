...
func GenerateKey() (SecretKey, PublicKey, error) {
	...
	for i := 0; i < 256; i++ {
		sec.ZeroPre[i] = generateRandomBlock()
		sec.OnePre[i] = generateRandomBlock()

		// Hash the public key and store the result in the secret key
		pub.ZeroHash[i] = hashBlock(sec.ZeroPre[i])
		pub.OneHash[i] = hashBlock(sec.OnePre[i])
	}

	return sec, pub, nil
}

// generateRandomBlock generates a random Block
...
// hashBlock hashes the given Block using SHA-256
func hashBlock(b Block) Block {
...
func Sign(msg Message, sec SecretKey) Signature {
	var sig Signature
	var counter, shift int

	for i := 0; i < 256; i++ {
		bitMessage := msg[counter]
		shiftedMessage := bitMessage << shift
		result := shiftedMessage & 0x80
...
func Verify(msg Message, pub PublicKey, sig Signature) bool {
	...
	for i := 0; i < 256; i++ {
		...
		if result == 0 {
			sign.Preimage[i] = pub.ZeroHash[i]
		} else {
			sign.Preimage[i] = pub.OneHash[i]
		}
		...
		hashSign.Preimage[i] = hashBlock(sig.Preimage[i])
		if hashSign.Preimage[i] != sign.Preimage[i] {
			return false
		}
	}
